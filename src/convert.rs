...
// 因为 Rust trait 的孤儿规则：当你为某类型实现某trait 的时候，必须要求类型或者trait 至少有一个是在当前crate 中定义的。 你不能为第三方的类型实现第三方的trait
// 如果想要对已有的类型实现已有的 trait，需要进行简单的包装
...
/// 把 SqlParser 解析出来的 Statement 转换为 Sql 结构
impl<'a> TryFrom<&'a Statement> for Sql<'a> {
    ...
    fn try_from(sql: &'a Statement) -> Result<Self, Self::Error> {
        match sql {
            ...
            _ => Err(anyhow!("We only support Quer at the moment")),
        }
    }
}
